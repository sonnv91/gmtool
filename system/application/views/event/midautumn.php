...
<section class="content">
        ...
        <div class="col-xs-12">
            <div class="box">
                <div>
                    ...
                    <!-- Tab panes -->
                    <form action="<?php echo base_url(); ?>data/event_midautumn/save" method="post" role="form">
                    <div class="tab-content">
                            <div role="tabpanel" class="tab-pane active" id="info">
                                ...
                                <div class="form-group">
                                    <label for="inputMapName" class="col-sm-2 control-label">Quà mốc 1</label>
                                    <div class="col-sm-10">
                                        <select name="reward_advanced[]" class="selectpicker" data-live-search="true">
                                            <?php foreach ($rewards as $reward) { ?>
                                                <option value="<?php echo $reward->id; ?>" <?php if ($reward->id == $config->rewardAdvanced[0]) {echo 'selected="selected"';}?>><?php echo $reward->name; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="inputMapName" class="col-sm-2 control-label">Quà mốc 2</label>
                                    <div class="col-sm-10">
                                        <select name="reward_advanced[]" class="selectpicker" data-live-search="true">
                                            <?php foreach ($rewards as $reward) { ?>
                                                <option value="<?php echo $reward->id; ?>" <?php if ($reward->id == $config->rewardAdvanced[1]) { echo 'selected="selected"';} ?>><?php echo $reward->name; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="inputMapName" class="col-sm-2 control-label">Quà mốc 3</label>
                                    <div class="col-sm-10">
                                        <select name="reward_advanced[]" class="selectpicker" data-live-search="true">
                                            <?php foreach ($rewards as $reward) { ?>
                                                <option value="<?php echo $reward->id; ?>" <?php if ($reward->id == $config->rewardAdvanced[2]) { echo 'selected="selected"';} ?>><?php echo $reward->name; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="inputMapName" class="col-sm-2 control-label">Điểm các mốc</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="point_advanced" class="form-control" value="<?php echo implode(",",$config->pointAdvanced); ?>">
                                    </div>
                                </div>
								<div class="form-group">
                                    <label for="inputMapName" class="col-sm-2 control-label">Số lần đổi mỗi ngày</label>
                                    <div class="col-sm-10">
                                        <input type="number" name="max_turn" class="form-control" value="<?php echo $config->maxTurnExchange; ?>">
                                    </div>
                                </div>
                            </div>
                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover" id="datatable">
                        ...
                        <?php $i=1;foreach($tops as $top){?>
                            <tr>
                                <td><?php echo $i;?></td>
                                <td><?php echo $top->owner;?></td>
                                <td><?php echo $top->point;?></td>
                                <td><?php echo DateUtil::formatTime($top->createTime->sec,'d-m-Y H:i:s');?></td>
                            </tr>
                        <?php $i++;}?>
                    </table>
                </div>
	...
	<button type="button" class="btn btn-success" id="viewTop">Top hôm trước</button>
	<div id="viewTop-content"></div>
...
<script>
	...
	$(document).on("click", "#viewTop", function(){
        $.ajax({
            url : 'http://gm.ganetstudio.com/crontab/topEventMidAutumn',
            type: "GET",
            data : {},
            success:function(data, textStatus, jqXHR)
            {
				$("#viewTop-content").html(data);
            },
